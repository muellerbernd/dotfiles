...
local sign_cache = {}
sign_cache[vim.diagnostic.severity.ERROR] = ' '
sign_cache[vim.diagnostic.severity.WARN] = ' '
sign_cache[vim.diagnostic.severity.INFO] = ' '
sign_cache[vim.diagnostic.severity.HINT] = ' '
-- sign_cache[vim.diagnostic.severity.ERROR] = ""
-- sign_cache[vim.diagnostic.severity.WARN] = ""
-- sign_cache[vim.diagnostic.severity.INFO] = ""
-- sign_cache[vim.diagnostic.severity.HINT] = ""
...
M.get_diag_counts = function()
  ...
  if grouped[S.ERROR] then
    result = result .. '%#StatusLineError#' .. get_sign(S.ERROR, true) .. grouped[S.ERROR] .. '%* '
  end
  if grouped[S.WARN] then
    result = result .. '%#StatusLineWarn#' .. get_sign(S.WARN, true) .. grouped[S.WARN] .. '%* '
  end
  if grouped[S.INFO] then
    result = result .. '%#StatusLineInfo#' .. get_sign(S.INFO, true) .. grouped[S.INFO] .. '%* '
  end
  if grouped[S.HINT] then
    result = result .. '%#StatusLineHint#' .. get_sign(S.HINT, true) .. grouped[S.HINT] .. '%* '
  end
  -- if grouped[S.ERROR] then
