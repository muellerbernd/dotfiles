...
usage() {
    ...
    echo "  no-fullscreen               unfullscreen the wl-mirror window"
    echo "  fullscreen-output [output]  set the output to fullscreen to (default asks via slurp)"
    echo "  no-fullscreen-output        set the output to fullscreen to to the current output"
...
case "$1" in
    ...
    no-fullscreen) mirror-cmd --no-fullscreen;;
